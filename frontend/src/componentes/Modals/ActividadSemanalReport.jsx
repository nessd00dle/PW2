          ...
          {Object.entries(datosEjemplo).map(([fandom, datos]) => (
            <div key={fandom} className="border-b border-white/10 pb-6 last:border-0">
              <div className="flex justify-between mb-3">
                ...
                <span className="text-[#56ab91] font-bold">Total: {datos.reduce((a, b) => a + b, 0)}</span>
              </div>
                ...
                {datos.map((valor, idx) => (
                  <div key={idx} className="flex-1 flex flex-col items-center gap-1">
                    <div className="relative w-full group">
                      <div
                        className="w-full bg-gradient-to-t from-[#56ab91] to-[#3a8a72] rounded-t-lg transition-all duration-300 hover:from-[#6bc4a8] hover:to-[#4a9a82] cursor-pointer"
                    ...
                    <span className="text-xs text-[#56ab91] font-semibold">{valor}</span>
                  </div>
                ))}
              </div>
            </div>
